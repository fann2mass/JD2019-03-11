...
public class PrintMath {
    public static void main(String[] args) {
        ...
        for (Method method : methods) {  //перебираем все методы класса Math, которые мы получили в строке 12
            if ((method.getModifiers() & Modifier.PUBLIC) == Modifier.PUBLIC) { // по заданию нам необходимо напечатать только публичные методы
                ...
                //Ниже способ номер два для методов — просто удаляем лишнее по заданию из строки, но привязываемся к текущему значению реплейса
        ...
        for (Field field : fields) { //перебираем все методы класса Math, которые мы получили в строке 13
            if ((field.getModifiers() & Modifier.PUBLIC) == Modifier.PUBLIC) {// по заданию нам необходимо напечатать только публичные поля
                ...
                //Ниже способ номер два для полей — просто удаляем лишнее по заданию из строки, но привязываемся к текущему значению реплейса
    ...
    private static String modifiers(int mods) {
        switch (mods) { // модификаторы хранятся одним числом в двоичном виде, но т.к. командой getModifiers мы получаем их значение в десятичной системе счисления,
            // то и свич привязываем к ней. Например первая единица в двоичной системе счисления отвечает за "public", значит все возвращаемые в десятичной системе счисления
            // нечетные числа будут указывать на публичный тип.
            ...
            default: // применялся во время выполнения для поиска неопределённых значений, так же реализован тут, что бы не было необходимости писать "return" вне свича.
                return null + " ";
        }
    }
    ...
    private static String parameters(int count, Method method) {
        switch (count) {//getParameterTypes - возвращает массив, в котором записаны
            case 1:
                return "(" + method.getParameterTypes()[0] + ")"; //если в метод передаётся 1 значение, выводит первое значение массива, которое имеет индекс [0]
            case 2:
                return "(" + method.getParameterTypes()[0] + "," + method.getParameterTypes()[1] + ")"; //если в метод передаётся 2 значение, выводит первое и второе значения массива,
            //которые имею индексы [0] и [1] соответственно.
            case 3:
                return "(" + method.getParameterTypes()[0] + "," + method.getParameterTypes()[1] + "," + method.getParameterTypes()[2] + ")"; // для текущего задания не нужен, просто так добавил
