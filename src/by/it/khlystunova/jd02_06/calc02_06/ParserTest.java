...
public class ParserTest {
    ...
    //проверяем вычисления со скалярами
    @Test
    public void scalarAddScalar() throws Exception {
        Var var = parser.calc("2+2");
        double expected = 4.0;
        double actual = Double.parseDouble(var.toString());
        assertEquals(expected, actual, 1e-6);//проверяет на равенство значений expected и actual с возможной погрешностью delta.
    }
    ...
    public void createVarFromMatrixCalc() throws Exception {
        ...
        String actual = VarFactory.createVar("A").toString();
        String expected = "{{8.0, 8.0}, {8.0, 8.0}}";
